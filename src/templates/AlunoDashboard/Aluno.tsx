...
import { NovaDuvida } from "@/components/AlunoDashboard/NovaDuvida/NovaDuvida";
import { FiltroCards } from "@/components/AlunoDashboard/FiltroCards/FiltroCards";
import { DuvidaCard } from "@/components/AlunoDashboard/DuvidaCard/DuvidaCard";
...
const AlunoDashboard = () => {
  ...
  const duvidasResolvidas = minhasDuvidas.filter(
    (d) => d.status === "concluida"
  ).length;
  const duvidasAceitas = minhasDuvidas.filter(
    (d) => d.status === "aceita"
  ).length;
  
  const stats = [
    { value: duvidasEnviadas, label: "Enviadas", filterValue: "Todas" },
    { value: duvidasAceitas, label: "Aceitas", filterValue: "aceita" },
    {
            ...
            <div className={styles.duvidasSection}>
              ...
              {minhasDuvidas
                .filter((duvida) => {
                  ...
                  if (filtro === "Resolvidas")
                    return duvida.status === "concluida";
              ...
              {minhasDuvidas.length === 0 && (
                <p className={styles.emptyMessage}>
                  Você ainda não possui dúvidas no banco de dados.
                </p>
              )}
            </div>
          </>
